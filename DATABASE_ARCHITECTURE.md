...
3. Create the implemented NOLOGIN group roles: `rwa_catalog_shadow_writer` for only the named Phase 1 identity/ingest/event tables, `rwa_signal_history_writer` for only the Signal checkpoint table, `rwa_listing_audit_reader` for select-only access to the Listing checkpoint and migration `0006`'s three safe publication views, `rwa_analytics_reader` for read-only analytics/reconciliation, and `rwa_alert_dispatcher` for only delivery/outbox dispatch. The migration owner is granted the two active writer groups so runtime transactions can immediately `SET LOCAL ROLE`; it is deliberately not granted `rwa_listing_audit_reader`. Before a Listing read cutover, provision a dedicated non-owner/non-superuser/non-writer login with only that reader membership and a separate connection URL. No role can write typed continuous market facts.
