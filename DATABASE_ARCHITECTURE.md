...
- Maintain the current `rwa-listing-audit-v2` bundle/read contract and `/api/listing-changes` read path. The writer adds a fixed publication-lease diagnostic and, whenever PostgreSQL is enabled, holds a 180-second database lease across durable mutation, a post-acquire cache checksum re-read, Runtime Cache publication, an immediate post-write bundle checksum verification and sink acknowledgement. It refreshes the same logical UTC-day bucket every six hours; this is continuity mitigation for an ephemeral cache, not four independent lifecycle days.
