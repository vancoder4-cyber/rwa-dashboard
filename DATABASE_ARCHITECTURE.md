...
Migration `0010` is an explicitly isolated Phase 2 exception for the arbitrage consumer contract. It does not activate the general Radar, reference, traditional or alert schemas. Migration `0011` bounds that slice: every executable candidate writes a compact exact-identity basis row for the rolling formula, while only policy-qualified routes write the wide fact. Each accepted five-minute capture is first-success-final for that bucket; an identical retry is idempotent and a different same-bucket payload is rejected rather than replacing history. Compact basis rows retain two hours, wide published-route facts retain six hours, and cleanup is fixed-scope/batched. The self-contained public snapshot remains append-only and only a Full ten-source collection can create it.

Phase 0 provisions the database and archive contract. Phase 1 shadow-writes the ten official Perpetual/Spot catalogs and the identity/listing lifecycle produced from them. Migration `0004` is a separately approved continuity repair: it stores four bounded, checksummed Signal history payloads in `publication.signal_history_checkpoint` so an ephemeral cache eviction cannot erase formula maturity. It is not a typed market-fact ingest or a full replay ledger. Apart from migration `0010`'s isolated arbitrage route/publication slice, continuous typed fact/aggregate writes, partition conversion and publication/alert execution still wait for later reviewed migrations.
...
| Listing audit | `analytics.catalog_change_event` plus trusted source runs; Runtime Cache/CDN are disposable read replicas | The same durable typed event authority plus later versioned publications | Phase 1 catalog dual-write; migration `0006` adds least-privilege safe views and an explicit `postgres-authoritative` read cutover |
| Arbitrage Bot publication | No valid API; the catch-all previously returned the HTML shell | Exact versioned route facts plus validated publication snapshot | Migration `0010`; authenticated five-minute writer, PostgreSQL authority, dedicated view-only reader, fail-closed public API |
...
Runtime Cache remains useful for low-latency latest snapshots but is disposable. The four Signal histories and the Listing writer state can recover from checksummed PostgreSQL checkpoints. For public Listing history, confirmed non-baseline rows in `analytics.catalog_change_event`, joined only to their versioned identity and trusted source-run coverage through safe publication views, are authoritative. The Listing checkpoint stores the writer's accepted compact comparison bundle verbatim; it is writer continuity state, not the lifecycle-event system of record. Neither the checkpoint nor current membership may be diffed after the fact to synthesize New/Re-listed events.
...
| `fact.arbitrage_basis_observation` | `basis_observation_id uuid`; unique cycle/route and bucket/fingerprint/formula | Compact exact asset + Spot + Perpetual version identity and basis needed by the rolling persistence formula | Migration `0011`; first-success-final five-minute buckets; two-hour hot retention, fixed 10,000-row cleanup batch |
| `fact.arbitrage_route_observation` | `route_observation_id uuid`; unique cycle/route and bucket/fingerprint/formula/input checksum | Exact asset + Spot instrument version + Perpetual instrument version; executable ask/bid side depth, OI, basis/persistence and exact-contract settled-funding summary for routes included in a public snapshot | Migrations `0010`/`0011`; first-success-final five-minute buckets; six-hour hot retention, fixed 5,000-row cleanup batch |
...
A versioned, **data-family-specific** collection policy is a Phase 2 prerequisite; there is no defensible universal elapsed-time or overlap gate. Phase 1 maintains one logical UTC-day official-catalog snapshot per source, with bounded same-day refreshes that replace that bucket, and shadow-writes identity/membership/lifecycle evidence. It does not fetch overlapping price, volume, OI, funding, reference or Traditional history and must not be cited as proof that market-history restatements have already been detected. The catalog-readiness fields `marketFactsChecked` and `rollingMarketHistoryVerified` therefore remain `false` throughout Phase 1.
...
|---|---|---|---|
| Official catalogs / competitor listings | Daily. Day 1 first Full run establishes a baseline only; Day 2 Full can emit New/Re-listed in `0–24h`. A verified delisting follows `D0 present → D1 first missing → D2 missing confirmed`, normally `24–48h` after the first complete missing observation. | The UI's default 7-day and expanded 45-day views are presentation/audit horizons, not warm-up requirements. | Current-catalog endpoints have no historical overlap: append each exact catalog capture and compare fingerprints. |
...
| `analytics.catalog_change_event` | `catalog_change_event_id uuid`; unique source/instrument/type/day; optional official time with coherent time basis | Durable Listing lifecycle-event authority; writer emits only verified non-baseline listed/delisted/relisted events and retains previous/current source-run evidence | Phase 1 write + migration `0006` authoritative read |
...
The `ops.*` tables after `ops.schema_migration` are documented later targets and are not created by `0002`. Generic alert/publication tables are empty skeletons in Phase 0/1; migration `0010` separately creates the dedicated arbitrage snapshot table/view. Outbox claims use `FOR UPDATE SKIP LOCKED`; `dedupe_key` is unique. A delivery provider failure must not roll back or mutate the underlying alert event.
...
3. Create the implemented NOLOGIN group roles: `rwa_catalog_shadow_writer` for only the named Phase 1 identity/ingest/event tables, `rwa_signal_history_writer` for only the Signal checkpoint table, `rwa_listing_audit_reader` for select-only access to the Listing checkpoint and migration `0006`'s three safe publication views, `rwa_arbitrage_writer` for only migration `0010`'s route facts/snapshot plus required lineage inserts, `rwa_arbitrage_reader` for only the frontend-safe arbitrage view, `rwa_analytics_reader` for read-only identity/ingest/fact/analytics/publication tables, and `rwa_alert_dispatcher` for only delivery/outbox dispatch. The migration owner is granted the active writer groups so runtime transactions can immediately `SET LOCAL ROLE`; it is deliberately not granted either dedicated public-reader role. Before a Listing or arbitrage read cutover, provision a dedicated non-owner/non-superuser/non-writer login with only the corresponding reader membership and a separate connection URL. No role may write typed continuous market facts outside the explicitly reviewed migration-`0010` slice.
...
7. Keep the implemented safe defaults `PG_WRITE_MODE=off`, `RAW_ARCHIVE_MODE=off`, `LISTING_CHECKPOINT_WRITE_MODE=off` and `LISTING_READ_MODE=runtime-cache`. `DATABASE_URL` configures the lazy pooled writer connection; `DATABASE_URL_UNPOOLED` is preferred for migrations and may fall back to the pooled URL only when intentionally configured. `LISTING_DATABASE_URL` and its Preview counterpart are dedicated read-only connections and cannot equal the writer URL. `BLOB_READ_WRITE_TOKEN` is private. Environment presence alone never enables a writer/reader, and browsers cannot control any switch.
...
- Maintain the current `rwa-listing-audit/v1` public compatibility contract. Migration `0005` stores the exact writer comparison bundle for continuity; migration `0006` instead projects confirmed lifecycle facts and trusted run coverage through least-privilege views. `LISTING_READ_MODE=postgres-authoritative` serves that typed projection and never consults Runtime Cache for event history. The writer still holds its 180-second database lease across durable mutation, checkpoint/Runtime Cache publication, immediate verification and sink acknowledgement. Four physical refreshes update one logical UTC-day bucket and never create four lifecycle days.
...
- A database outage while `LISTING_READ_MODE=runtime-cache|dual-read` must not replace the current Runtime Cache result; dual-read exposes the failed reconciliation. Under `durable-fallback`, a missing or invalid checkpoint fails closed. Under separately approved `postgres-authoritative`, a missing/invalid reader, source-run set or event projection fails closed and must never fall back to an empty cache as “no events.”
