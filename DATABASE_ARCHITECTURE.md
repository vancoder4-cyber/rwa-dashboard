...
Migration `0010` is an explicitly isolated Phase 2 exception for the arbitrage consumer contract. It does not activate the general Radar, reference, traditional or alert schemas. Each accepted five-minute route capture is append-only and first-success-final for that bucket; an identical retry is idempotent and a different same-bucket payload is rejected rather than replacing history. The public snapshot is also append-only and only a Full ten-source collection can create it.
