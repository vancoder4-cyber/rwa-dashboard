...
| Listing audit | Exact bounded `publication.listing_audit_checkpoint` plus `rwa-listing-audit-v2` Runtime Cache replica | Durable catalog memberships/events plus later versioned publications | Phase 1 catalog dual-write; checkpoint/read switches default off/runtime-cache and require a separate staged cutover |
...
Runtime Cache remains useful for low-latency latest snapshots but is disposable. The four Signal histories and the exact Listing Audit publication bundle can recover from separate checksummed PostgreSQL checkpoints. The Listing checkpoint stores the writer's accepted compact bundle verbatim and is never reconstructed from catalog membership or lifecycle tables. A bounded checkpoint is continuity storage, not a substitute for typed facts, a complete audit ledger or raw archives.
...
3. Create the implemented NOLOGIN group roles: `rwa_catalog_shadow_writer` for only the named Phase 1 identity/ingest/event tables, `rwa_signal_history_writer` for only the Signal checkpoint table, `rwa_listing_audit_reader` for select-only access to the single Listing checkpoint, `rwa_analytics_reader` for read-only analytics/reconciliation, and `rwa_alert_dispatcher` for only delivery/outbox dispatch. The migration owner is granted the two active writer groups so runtime transactions can immediately `SET LOCAL ROLE`; it is deliberately not granted `rwa_listing_audit_reader`. Before a Listing dual-read/fallback, provision a dedicated non-owner/non-superuser login with only that reader membership and a separate connection URL. No role can write typed continuous market facts.
...
- Maintain the current `rwa-listing-audit-v2` bundle contract. The default `/api/listing-changes` path remains Runtime Cache. Migration `0005` may shadow-store that exact bundle in a bounded checkpoint; `dual-read` never changes the served source, while separately approved `durable-fallback` requires the validated checkpoint and fails closed on cache-ahead or checksum divergence. The writer adds a fixed publication-lease diagnostic and, whenever PostgreSQL is enabled, holds a 180-second database lease across durable mutation, a post-acquire configured-reader re-selection, checkpoint/Runtime Cache publication, an immediate post-write bundle checksum verification and sink acknowledgement. It refreshes the same logical UTC-day bucket every six hours; this is continuity mitigation for an ephemeral cache, not four independent lifecycle days.
...
- A database outage while `LISTING_READ_MODE=runtime-cache|dual-read` must not replace the current Runtime Cache result; dual-read exposes the failed reconciliation. Under separately approved `durable-fallback`, a missing or invalid checkpoint fails closed, while a valid last-good exact publication may be served with explicit read source, age and status only within the existing Listing freshness contract.
