...
6. Request the same normalized Gate bulk URL and the fixed Binance 30d, trade.xyz 30d, OKX `perp-snapshot`/`spot-snapshot`, and U.S. directory URLs twice. After a cold `MISS` (or an already warm response), the second response must be `HIT`/`STALE`; validation and total-upstream 4xx/5xx responses must be `no-store`.
