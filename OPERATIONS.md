...
2. **Vercel health probe**
   - `GET /api/health` verifies the production shell, USD and FX-converted Reference Prices, the official U.S. listing directory, and 24-hour Funding History sentinels for trade.xyz, Bitget, Gate, Binance and OKX. The U.S. directory check requires AAPL/QQQ/BABA/TSM and validates ADR membership, but never requires a ticker to remain absent forever. Legitimate securities whose tickers also exist in Crypto are permitted in the directory; those collisions and Pre-IPO identities are rejected earlier by venue/category gates. The OKX probe also checks the fixed-purpose Perp/Spot snapshots, official identity gate, listing/canonical baselines and mandatory ticker/OI/mark coverage.
...
| U.S. listing identity | One fixed `/api/us-market-directory` shape; no symbol/query variants. The browser refreshes the shared full snapshot hourly and retries initial failures with 5–30 minute backoff. The server projects both Nasdaq Trader text directories to sorted symbol/ADR arrays and validates both raw source epochs against their strict `America/New_York` timestamps. | Nominally 1-hour fresh + up to 6-hour stale-while-revalidate, but both windows are shortened near expiry so no CDN object can cross the seven-day source hard limit. An initial upstream/schema failure is 503 + no-store and disables the US filter; after one verified snapshot, a refresh failure keeps that last-good filter usable and visibly marks it stale only until that same absolute expiry. |
