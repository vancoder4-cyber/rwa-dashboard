(function () {
  ...
  var ZH = Object.freeze({
    ...
    'Compares current rolling 24h USD contract volume with the prior seven sealed UTC-date rolling-24h observations for the same exact venue-instrument cohort. HIGH ≥ 2× · MEDIUM ≥ 1.5× · DOWN ≤ 0.4×.':'将当前滚动 24 小时美元合约成交量，与同一精确平台标的组合过去七个已封存 UTC 日期的滚动 24 小时观察值均量进行比较。HIGH ≥ 2× · MEDIUM ≥ 1.5× · DOWN ≤ 0.4×。',
    'Open Signal Radar to load server-side contract-volume anomaly history.':'打开 RWA 信号雷达以加载服务端合约成交量异动历史。',
    'Anomaly levels, 7-day baselines, 30-day frequency and consecutive-expansion flags are supplied by the server. Missing fields or changing venue coverage never become DOWN alerts.':'异动级别、7 日基线、30 日频率及连续放量标记均由服务端提供。字段缺失或平台覆盖变化不会被误判为 DOWN。',
    'Filter volume anomaly level':'筛选成交量异动级别',
    'All anomaly levels':'全部异动级别',
    'HIGH':'高',
    'MEDIUM':'中',
    'DOWN':'下降',
    'Filter volume anomaly category':'筛选成交量异动类别',
    'Filter volume anomaly venue':'筛选成交量异动平台',
    'Filter volume anomaly data status':'筛选成交量异动数据状态',
    'All data statuses':'全部数据状态',
    'Sort volume anomalies':'成交量异动排序',
    'Alert priority':'异动优先级',
    'Volume ratio':'成交量倍数',
    'Current 24h volume':'当前 24 小时成交量',
    '30d anomaly frequency':'近 30 天异动频率',
    'Search volume anomalies...':'搜索成交量异动…',
    'Search volume anomaly assets':'搜索成交量异动资产',
    'This section displays only server-classified contract-volume anomalies.':'本板块仅展示服务端已分类的合约成交量异动。',
    'High-frequency':'高频异动',
    '≥2× 7d avg':'≥2× 7 日均量',
    '≥1.5× 7d avg':'≥1.5× 7 日均量',
    '≤0.4× · liquidity warning':'≤0.4× · 流动性警示',
    '30d server flag':'近 30 天服务端标记',
    'Loading server-side contract-volume anomalies…':'正在加载服务端合约成交量异动…',
    'Perpetual volume anomaly baseline is warming. No absence conclusion is available until seven comparable daily observations are ready.':'合约成交量异动基线正在预热。积累七个可比日观察值之前，不能得出“没有异动”的结论。',
    'Perpetual volume anomaly history is unavailable. No HIGH, MEDIUM or DOWN conclusion is available.':'合约成交量异动历史不可用，无法得出 HIGH、MEDIUM 或 DOWN 结论。',
    'Perpetual volume anomaly coverage is Partial. Known alerts remain visible; missing coverage cannot support a no-anomaly conclusion.':'合约成交量异动覆盖为部分可用。已知提醒继续展示，但缺失的覆盖不足以支持“没有异动”的结论。',
    'No perpetual volume anomalies in this snapshot':'本次快照没有合约成交量异动',
    'Server-side contract-volume anomaly classification is Full.':'服务端合约成交量异动分类完整。',
    'No known volume anomalies match the current filters':'没有符合当前筛选条件的已知成交量异动',
    'Adjust the level, category, venue, data status or search filters.':'请调整级别、类别、平台、数据状态或搜索条件。',
    'Adjust the filters. Coverage is incomplete, so this is not a no-anomaly conclusion.':'请调整筛选条件。当前覆盖不完整，因此不能据此判断没有异动。',
    'All monitored assets have comparable server history for this snapshot.':'本次快照中所有受监控资产均具备可比的服务端历史。',
    'Perpetual volume anomaly coverage is Partial':'合约成交量异动覆盖为部分可用',
    'No absence conclusion is available while monitored assets or frequency history remain incomplete.':'受监控资产或频率历史尚不完整时，不能得出“没有异动”的结论。',
    'Perpetual volume anomaly history is unavailable':'合约成交量异动历史不可用',
    'No HIGH, MEDIUM or DOWN conclusion is available.':'无法得出 HIGH、MEDIUM 或 DOWN 结论。',
    'Perpetual volume anomaly baseline is warming':'合约成交量异动基线正在预热',
    'Seven comparable daily observations are required before the first classification.':'首次分类前需要七个可比日观察值。',
    'Current 24h Volume':'当前 24 小时成交量',
    '7d Avg Volume':'7 日平均成交量',
    'vs 7d Avg':'较 7 日均量',
    '30d Anomaly Frequency':'近 30 天异动频率',
    'Data Status / Baseline':'数据状态 / 基线',
    'Alert':'异动',
    'Liquidity warning':'流动性警示',
    'Volume expansion':'成交量放大',
    'High-frequency anomaly':'高频异动',
    'Baseline unavailable':'基线不可用',
    'Frequency':'频率',
    'Frequency ·':'频率 ·',
    'perpetual volume anomalies':'合约成交量异动',
    'RWA Spot Volume & Price Anomalies':'RWA 现货量价异动',
    'Spot source coverage':'现货数据源覆盖',
    'Spot sources not loaded.':'现货数据源尚未加载。',
    'coverage unavailable':'覆盖不可用',
    'Monitors verified RWA spot venue instruments. An alert is raised when current rolling 24h USD volume is at least 3× its prior-day comparable anchor OR the official 24h price gain is at least +15%. Instruments below $500K current 24h volume are excluded.':'监控已验证的 RWA 现货平台标的。当当前滚动 24 小时美元成交量至少达到前一日可比锚点的 3 倍，或官方 24 小时涨幅至少达到 +15% 时触发提醒；当前 24 小时成交量低于 50 万美元的标的会被剔除。',
    'Open Signal Radar to load server-side spot volume and price anomalies.':'打开 RWA 信号雷达以加载服务端现货量价异动。',
    'The server applies the two alert conditions with OR logic after the $500K liquidity filter. The prior-day volume anchor is a comparable sealed observation, while exact perpetual venue contracts are joined by verified category and canonical underlying.':'服务端先应用 50 万美元流动性过滤，再以“或”逻辑判断两项提醒条件。前一日成交量锚点是可比的已封存观察值，永续合约则按已验证类别与标准底层资产精确关联到平台标的。',
    'Filter spot anomaly trigger':'筛选现货异动触发条件',
    'All spot anomaly triggers':'全部现货异动触发条件',
    'Volume ≥ 3×':'成交量 ≥ 3×',
    '24h gain ≥ 15%':'24 小时涨幅 ≥ 15%',
    'Both conditions':'同时满足两项',
    'Filter spot anomaly category':'筛选现货异动类别',
    'Filter spot anomaly venue':'筛选现货异动平台',
    'All spot venues':'全部现货平台',
    'Filter perpetual listing status':'筛选永续合约上线状态',
    'All perpetual statuses':'全部永续合约状态',
    'Perpetual listed':'已上线永续合约',
    'No perpetual listing':'未上线永续合约',
    'Perpetual coverage unknown':'永续合约覆盖未知',
    'Filter spot anomaly data status':'筛选现货异动数据状态',
    'Sort spot anomalies':'现货异动排序',
    '24h price gain':'24 小时涨幅',
    'Search spot anomalies or contracts...':'搜索现货异动或合约…',
    'Search spot anomalies or contracts':'搜索现货异动或合约',
    'This section displays only server-classified spot volume or price anomalies above the liquidity floor.':'本板块仅展示通过流动性门槛且由服务端分类的现货成交量或价格异动。',
    'Alert instruments':'异动标的',
    'OR logic':'“或”逻辑',
    'vs prior-day anchor':'较前一日锚点',
    '$500K floor':'50 万美元门槛',
    'exact venue contracts':'精确平台合约',
    'Loading server-side spot volume and price anomalies…':'正在加载服务端现货量价异动…',
    'Spot prior-day volume baseline is warming. Known price-surge alerts remain visible, but no all-clear conclusion is available yet.':'现货前一日成交量基线正在预热。已知价格大涨提醒继续展示，但目前不能得出“没有异动”的结论。',
    'Spot volume and price anomaly data is unavailable. No alert or no-alert conclusion is available.':'现货量价异动数据不可用，无法得出“有提醒”或“没有提醒”的结论。',
    'Spot volume and price anomaly coverage is Partial. Known alerts remain visible; missing spot, history or perpetual coverage cannot support a no-alert conclusion.':'现货量价异动覆盖为部分可用。已知提醒继续展示，但缺失的现货、历史或永续合约覆盖不足以支持“没有提醒”的结论。',
    'No qualifying RWA spot volume or price anomalies in this snapshot':'本次快照没有符合条件的 RWA 现货量价异动',
    'Server-side RWA spot volume and price anomaly classification is Full.':'服务端 RWA 现货量价异动分类完整。',
    'No known spot anomalies match the current filters':'没有符合当前筛选条件的已知现货异动',
    'Adjust the trigger, category, spot venue, perpetual status, data status or search filters.':'请调整触发条件、类别、现货平台、永续合约状态、数据状态或搜索条件。',
    'Adjust the filters. Coverage is incomplete, so this is not a no-alert conclusion.':'请调整筛选条件。当前覆盖不完整，因此不能据此判断没有提醒。',
    'No qualifying RWA spot anomalies in this snapshot':'本次快照没有符合条件的 RWA 现货异动',
    'All monitored venue instruments were evaluated after the $500K current-volume floor.':'所有受监控的平台标的都已在 50 万美元当前成交量门槛后完成评估。',
    'Spot anomaly coverage is Partial':'现货异动覆盖为部分可用',
    'Known alerts remain visible, but incomplete spot, history or perpetual coverage prevents an all-clear conclusion.':'已知提醒继续展示，但现货、历史或永续合约覆盖不完整，因此不能得出“没有异动”的结论。',
    'Spot anomaly data is unavailable':'现货异动数据不可用',
    'No alert or no-alert conclusion is available.':'无法得出“有提醒”或“没有提醒”的结论。',
    'Spot prior-day volume baseline is warming':'现货前一日成交量基线正在预热',
    'Price-surge alerts can appear immediately; volume-ratio alerts require a comparable prior-day anchor.':'价格大涨提醒可立即出现；成交量倍数提醒需要可比的前一日锚点。',
    'VOLUME ≥ 3×':'成交量 ≥ 3×',
    'PRICE ≥ 15%':'价格 ≥ 15%',
    'BOTH':'两项同时触发',
    'OR condition · server classified':'“或”条件 · 服务端分类',
    'No perpetual listing ·':'未上线永续合约 ·',
    'Perpetual coverage unknown ·':'永续合约覆盖未知 ·',
    'Liquidity floor · $500K':'流动性门槛 · 50 万美元',
    'Asset / Spot Instrument':'资产 / 现货标的',
    'Trigger':'触发条件',
    'Prior-day 24h Anchor':'前一日 24 小时锚点',
    'Volume Ratio':'成交量倍数',
    '24h Price Gain':'24 小时涨幅',
    'Perpetual Contracts':'永续合约',
    'spot volume and price anomalies':'现货量价异动',
    'verified server Spot anomaly snapshot':'已验证的服务端现货异动快照',
    'OI Positioning & Large-Liquidation Proxy':'OI 持仓与大额清算代理',
    'Tracks server-classified open-interest positioning, completed-day OI trends and rapid OI drawdowns across verified RWA perpetual markets. “Large-liquidation proxy” is a deleveraging proxy—not a trade-by-trade liquidation feed.':'监控已验证 RWA 永续市场中由服务端分类的未平仓量持仓、已完成交易日 OI 趋势及快速 OI 回撤。“大额清算代理”属于去杠杆代理指标，并非逐笔清算数据源。',
    'Open Signal Radar to load server-side OI positioning and large-liquidation proxy alerts.':'打开 RWA 信号雷达以加载服务端 OI 持仓与大额清算代理提醒。',
    'All alerts, trends and trader bias labels are supplied by the server. OI USD drawdown is also affected by mark-price changes, so the proxy cannot identify which long or short positions were liquidated and must not be read as actual large-liquidation prints.':'所有提醒、趋势及大户方向标签均由服务端提供。OI 美元价值回撤也会受到标记价格变化影响，因此该代理指标无法判断具体是多头还是空头被清算，也不能解读为真实的大额清算逐笔记录。',
    'OI source observation coverage':'OI 数据源观测覆盖',
    'OI sources not loaded.':'OI 数据源尚未加载。',
    'Official source observation coverage':'官方数据源观测覆盖',
    'Filter OI and liquidation proxy alert type':'筛选 OI 与清算代理提醒类型',
    'All OI alert types':'全部 OI 提醒类型',
    'Filter OI and liquidation proxy category':'筛选 OI 与清算代理类别',
    'Filter OI and liquidation proxy venue':'筛选 OI 与清算代理平台',
    'Filter Binance top trader bias':'筛选 Binance 大户方向',
    'All trader biases':'全部大户方向',
    'Bullish':'偏多',
    'Bearish':'偏空',
    'Neutral':'中性',
    'Mixed':'混合',
    'Filter OI and liquidation proxy data status':'筛选 OI 与清算代理数据状态',
    'Sort OI and liquidation proxy alerts':'OI 与清算代理提醒排序',
    'Current OI':'当前 OI',
    'Peak drawdown':'峰值回撤',
    'Top trader ratio':'大户持仓比',
    'Search OI and liquidation proxy alerts...':'搜索 OI 与清算代理提醒…',
    'Search OI and liquidation proxy alerts':'搜索 OI 与清算代理提醒',
    'This section displays only server-classified OI positioning and large-liquidation proxy alerts.':'本板块仅展示由服务端分类的 OI 持仓与大额清算代理提醒。',
    'OI and liquidation anomalies':'OI 与清算代理异动',
    'oi and liquidation anomalies':'OI 与清算代理异动',
    'verified server OI and liquidation proxy snapshot':'已验证的服务端 OI 与清算代理快照',
    'Large-liquidation proxy':'大额清算代理',
    'OI rising':'OI 连续上升',
    'OI rising + large-liquidation proxy':'OI 连续上升 + 大额清算代理',
    'Deleveraging proxy':'去杠杆代理',
    'not trade-by-trade liquidations':'并非逐笔清算',
    'Alert assets':'提醒资产',
    'server-classified only':'仅服务端分类',
    '3 completed UTC days':'3 个已完成 UTC 日',
    'Binance positioning':'Binance 大户持仓',
    'alert assets with exact data':'具备精确数据的提醒资产',
    'Loading server-side OI positioning and large-liquidation proxy alerts…':'正在加载服务端 OI 持仓与大额清算代理提醒…',
    'OI and liquidation-proxy history is warming. No absence conclusion is available until the server marks history ready.':'OI 与清算代理历史正在预热。服务端将历史标记为就绪之前，不能得出“没有异动”的结论。',
    'OI positioning and large-liquidation proxy data is unavailable. No alert or no-alert conclusion is available.':'OI 持仓与大额清算代理数据不可用，无法得出“有提醒”或“没有提醒”的结论。',
    'OI positioning and large-liquidation proxy coverage is Partial. Known alerts remain visible; incomplete fields or history cannot support a no-anomaly conclusion.':'OI 持仓与大额清算代理覆盖为部分可用。已知提醒继续展示，但字段或历史不完整，不能支持“没有异动”的结论。',
    'No OI positioning or large-liquidation proxy anomalies in this Full snapshot':'本次完整快照没有 OI 持仓或大额清算代理异动',
    'Server-side OI positioning and large-liquidation proxy classification is Full.':'服务端 OI 持仓与大额清算代理分类完整。',
    'Rising':'上升',
    'Not rising':'未连续上升',
    'No known OI or liquidation proxy alerts match the current filters':'没有符合当前筛选条件的已知 OI 或清算代理提醒',
    'Adjust the alert type, category, venue, trader bias, data status or search filters.':'请调整提醒类型、类别、平台、大户方向、数据状态或搜索条件。',
    'Adjust the filters. Coverage or history is incomplete, so this is not a no-anomaly conclusion.':'请调整筛选条件。当前覆盖或历史不完整，因此不能据此判断没有异动。',
    'No OI positioning or large-liquidation proxy anomalies in this snapshot':'本次快照没有 OI 持仓或大额清算代理异动',
    'The server reports Full source coverage and ready OI history. The proxy is not a trade-by-trade liquidation feed.':'服务端报告数据源覆盖完整且 OI 历史已就绪。该代理指标并非逐笔清算数据源。',
    'OI positioning and liquidation proxy data is unavailable':'OI 持仓与清算代理数据不可用',
    'OI positioning and liquidation proxy coverage is Partial':'OI 持仓与清算代理覆盖为部分可用',
    'Known alerts remain visible, but incomplete OI, volume, trader positioning or history prevents an all-clear conclusion.':'已知提醒继续展示，但 OI、成交量、大户持仓或历史不完整，因此不能得出“没有异动”的结论。',
    'OI and liquidation-proxy history is warming':'OI 与清算代理历史正在预热',
    'The server must finish the required hourly and completed-day history before an absence conclusion is available.':'服务端完成所需的小时级与已完成日历史之前，不能得出“没有异动”的结论。',
    '3 Completed-Day OI Closes / Trend':'最近 3 个已完成日 OI 收盘值 / 趋势',
    '24h OI Peak':'24 小时 OI 峰值',
    'Peak Drawdown':'峰值回撤',
    'Binance Top Trader Position Ratio / Bias':'Binance 大户持仓比 / 方向',
    'Server classified':'服务端分类',
    'Ratio —':'持仓比 —',
    'History':'历史',
  ...
  var COMPOSITE_PATTERNS = [
    ...
    [/^Vol (\d[\d,]*)\/(\d[\d,]*) · Price (\d[\d,]*)\/(\d[\d,]*)$/, function (m) {
      return '成交量 ' + m[1] + '/' + m[2] + ' · 涨跌幅 ' + m[3] + '/' + m[4];
    }],
    [/^OI (\d[\d,]*)\/(\d[\d,]*) · Vol (\d[\d,]*)\/(\d[\d,]*)$/, function (m) {
      return 'OI ' + m[1] + '/' + m[2] + ' · 成交量 ' + m[3] + '/' + m[4];
    }]
  ];
  ...
  var PATTERNS = [
    ...
    [/^(\d[\d,]*) \/ (\d[\d,]*) eligible days$/, function (m) { return m[1] + ' / ' + m[2] + ' 个有效日'; }],
    [/^(\d[\d,]*)-day expansion streak$/, function (m) { return '连续 ' + m[1] + ' 天放量'; }],
    [/^Baseline (\d[\d,]*)\/(\d[\d,]*) days$/, function (m) { return '基线 ' + m[1] + '/' + m[2] + ' 天'; }],
    [/^(\d[\d,]*) listings$/, function (m) { return m[1] + ' 个上市标的'; }],
    ...
    [/^Perpetual volume anomaly refresh failed \((.+)\)\. Last-known alerts remain visible, but the cached snapshot cannot support a no-anomaly conclusion\.$/, function (m) { return '合约成交量异动刷新失败（' + m[1] + '）。仍显示最近一次已知提醒，但缓存快照不能支持“没有异动”的结论。'; }],
    [/^Spot volume and price anomaly refresh failed \((.+)\)\. Last-known alerts remain visible, but the cached snapshot cannot support a no-alert conclusion\.$/, function (m) { return '现货量价异动刷新失败（' + m[1] + '）。仍显示最近一次已知提醒，但缓存快照不能支持“没有提醒”的结论。'; }],
    [/^OI and liquidation proxy refresh failed \((.+)\)\. Last-known alerts remain visible, but the cached snapshot cannot support a no-anomaly conclusion\.$/, function (m) { return 'OI 与清算代理刷新失败（' + m[1] + '）。仍显示最近一次已知提醒，但缓存快照不能支持“没有异动”的结论。'; }],
    [/^Long (\d+(?:\.\d+)?)%$/, function (m) { return '多头 ' + m[1] + '%'; }],
    [/^Short (\d+(?:\.\d+)?)%$/, function (m) { return '空头 ' + m[1] + '%'; }],
    [/^Ratio (\d+(?:\.\d+)?)$/, function (m) { return '持仓比 ' + m[1]; }],
    [/^(\d[\d,]*) meet both conditions$/, function (m) { return m[1] + ' 个同时满足两项条件'; }],
    [/^(\d[\d,]*) below \$500K excluded$/, function (m) { return '已剔除 ' + m[1] + ' 个低于 50 万美元的标的'; }],
    [/^Listed · (\d[\d,]*) contracts?$/, function (m) { return '已上线 · ' + m[1] + ' 个合约'; }],
