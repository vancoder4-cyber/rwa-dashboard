...
| UI window | Perp Aggregator shows only Perp and Spot Overview only Spot; rolling seven days and at most five rows by default, “View all” uses 45 days; pending reviews are not event-window-truncated |
...
| Time | `observedAt` is first system confirmation; a reliable official catalog timestamp may additionally populate `officialListedAt` and `timeBasis=official`, but cannot create an event by itself |
| Empty state | “No events” is allowed only with a fresh trusted source-run result; Warming/Unavailable remains unknown, while Partial shows reliable events with incomplete-coverage disclosure |
...
| Arbitrage opportunities | `/api/arbitrage-opportunities` CDN read of a validated full snapshot | `fact.arbitrage_basis_observation` (2h compact working set) + `fact.arbitrage_route_observation` (6h published-route facts) + append-only `publication.arbitrage_opportunity_snapshot` | **Yes, isolated Phase 2 slice through migrations `0010`/`0011`; dedicated reader view** |
