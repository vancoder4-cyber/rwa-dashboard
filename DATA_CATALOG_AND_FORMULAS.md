...
Current publication is the compact Runtime Cache namespace `rwa-listing-audit-v2`: target 45 event days, maximum 2,000 events, inactive identity retention 180 days and 1.75 MB guard. Migration `0005` adds a single checksummed PostgreSQL continuity checkpoint for that exact already-accepted bundle; it is default-off, cycle-linked and never reconstructed from catalog membership. Runtime Cache remains the low-latency replica. `dual-read` observes without changing the served source; separately approved `durable-fallback` can serve only a valid same-contract checkpoint when Runtime Cache is missing, unavailable or behind. Cache-ahead and equal-time checksum divergence fail closed. The 7/30-day UI windows and 45-day retention are view/audit horizons, not product warm-up. Truncation is disclosed and cannot be Full or “no new assets.” Constants: `api/_lib/listing-audit.js:1-22`, checkpoint contract: `api/_lib/listing-audit-checkpoint.js`.
...
| Competitor Listings | CDN 5m/10m public read; Runtime Cache replica | Exact bounded PostgreSQL checkpoint + Runtime Cache, 45-day target / 2,000 event cap | **Yes, Phase 1 shadow + publication continuity checkpoint** |
