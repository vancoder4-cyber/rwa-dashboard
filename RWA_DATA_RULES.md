...
- 五源身份与字段覆盖可以达到 Full，但跨 listing 聚合后的 OI USD、三个日收盘、24 小时峰值、峰值回撤及由它们派生的 signal 字段均必须标 **Estimated**；Full coverage 不能被展示成“精确 liquidation”。
...
- 价格、成交量、OI、Funding、深度、Top Trader、Reference、Traditional activity、canonical aggregate、Radar/异动信号与 publication snapshot 在 Phase 0/1 只允许创建目标 schema/文档骨架，**不得开始持续写入或切换读取**。这些属于后续独立阶段，必须重新做公式守恒、容量、回放与回滚验收。
